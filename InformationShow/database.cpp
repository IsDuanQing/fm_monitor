    ...
    ui(new Ui::database)
{
    ...
    if (ok){
     ...
     ui->comboBox->addItem("mq9");
     ui->comboBox->addItem("flour");
     ui->comboBox->addItem("beepLed");
     ui->comboBox->addItem("db");
     /*设置日期查询的初始化*/
...
void database::SelectAllPushTableData(){
    ...
    QString strSelectData = "select * from data";
    tableModel->setQuery(strSelectData);

    ui->tableView->setModel(tableModel);
}

void database::UpdataToDataBase(QString Stemp,QString Shumi,QString Smq9,QString Sflour,QString Sdb,QString SbeepLed)
{
    float val[6];
    ...
    val[1] = Shumi.toFloat();
    val[2] = Smq9.toFloat();
    val[3] = Sflour.toFloat();
    val[4] = SbeepLed.toFloat();
    val[5] = Sdb.toFloat();
...
{
    QSqlQuery query;
    QString stInsertData = "insert into data values (?,?,?,?,?,?,?)";
...
{
    QSqlQuery query;
    QString strClearDB = "delete from data";
...
void database::SelectData()
{
    ...
    //查询操作
    QString strSelectData = "select *from data where CurrenTime between '"+startTime+"' and '"+startTime2+"';";
    tableModel->setQuery(strSelectData);
    ui->tableView->setModel(tableModel);
}
...
void database::on_select_val_bt_clicked()
{
    ...
    //查询操作
    QString strSelectData = "select *from data where "+strNum+" >= "+downNum+" && "+strNum+"<="+upNum+";";
